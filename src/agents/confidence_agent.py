...
Assesses how confident we should be in a given prediction.
Includes probability calibration and model disagreement detection.
"""
...
class ConfidenceAgent:
    """
    Evaluates prediction reliability based on:
    - Model agreement (LR vs XGBoost)
    - Elo-based expected outcome strength
    - Data completeness
    - Recent performance volatility
    - Probability calibration (compress overconfidence)
    """

    def __init__(self):
        # Fix #1: Calibration settings
        self.calibration_ceiling = 0.75
        self.calibration_compression = 0.4

        # Fix #5: Model disagreement threshold
        self.disagreement_threshold = 0.20

    def calibrate_probability(self, p):
        """
        Fix #1: Compress extreme probabilities to reduce overconfidence.
        Maps probabilities above the ceiling into a compressed range.
        Example: 0.86 -> 0.75 + (0.86 - 0.75) * 0.4 = 0.794
        """
    ...
    def check_model_disagreement(self, lr_prob, xgb_prob):
        """
        Fix #5: Detect when LR and XGBoost disagree significantly.
        Returns disagreement magnitude and whether to downgrade confidence.
        """
    ...
    def score(self, prediction, game_features=None):
        """
        Compute confidence score (0-1) and label.

        Args:
            prediction: Output from PredictionAgent.predict()
            game_features: Optional dict of game-level features

        Returns:
            dict with confidence_score, confidence_label, breakdown,
                  calibrated_prob, and betting recommendation
        """
        breakdown = {}
        ...
        # ── 5. Volatility Penalty (0-1, higher is calmer) ──
        ...
        weights = {
            "model_agreement": 0.30,   # Increased from 0.25 (Fix #5)
        ...
        # ── Fix #5: Downgrade if models conflict ──
